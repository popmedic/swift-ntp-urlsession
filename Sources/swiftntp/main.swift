...
let task = URLSession.shared.dataTask(with: request) { data, _, error in
    ...
    print(tty.wrap("Localhost Date: \(localDate.description(with: .current))", inColor: .cyan))
    print(tty.wrap("Apple.com Date: \(date.description(with: .current))", inColor: .green))
    print(tty.wrap("Difference: \(date.timeIntervalSince1970 - localDate.timeIntervalSince1970)", inColor: .red))
    group.leave()
}
// enter the group
group.enter()
